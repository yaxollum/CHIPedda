...
std::string Disassembler::disassemble() {
  ...
  for (size_t i = 0; i < program.size() - 1;
       i += 2) // loop up to the second-last byte of program
  {
...
std::string Disassembler::disassembleInstruction(
    uint8_t byte1, uint8_t byte2) // diassemble next instruction
{
  ...
  case 0x8:
    ...
    case 0x6: // 8xy6 - SHR Vx {, Vy}
      assemblyStream << "SHR " << nibble2_reg << ", V0";
      break;
    case 0x7: // 8xy7 - SUBN Vx, Vy
      assemblyStream << "SUBN " << nibble2_reg << ", " << nibble3_reg;
      break;
    case 0xe: // 8xyE - SHL Vx {, Vy}
      assemblyStream << "SHL " << nibble2_reg << ", V0";
      break;
    default:
      throw UnsupportedInstructionException(full);
    }
  ...
  case 0xd: // Dxyn - DRW Vx, Vy, nibble
    assemblyStream << "DRW " << nibble2_reg << ", " << nibble3_reg << ", "
                   << nibble4_str;
    break;
  case 0xe:
    switch (byte2) {
